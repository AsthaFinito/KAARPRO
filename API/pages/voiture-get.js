...
exports.contactPost = async function(req, res) {
    ...
    try {
        db = await MongoClient.connect(url);
        let dbo = db.db("voiture");
        await dbo.collection("message").insertOne(mail);
...
exports.contactGet = async function(req, res) {
    try {
        db = await MongoClient.connect(url);
        let dbo = db.db("voiture");
        let datas = await dbo.collection("message").find({}).toArray();
        res.status(200).json(datas);
    } catch (err) {
        console.log(err);
        res.status(500).json({ message: err })
...
exports.contactDelete = async function(req, res, next) {
    try {
        console.log("test delete contact")
        db = await MongoClient.connect(url);
        let dbo = db.db("voiture");
        await dbo.collection("message").deleteOne({ _id: new mongodb.ObjectId(req.params.id) });
        res.status(200).send();
    } catch (err) {
        console.log(err);
        res.status(500).json({ message: err })
